...
def get_especie_by_name():
    ...
    # Busca sense distingir majúscules/minúscules ni accents:
    # Assegura't que la teva BD suporta utf8mb4_0900_ai_ci (MySQL 8+)
    sql = """
        SELECT ID, nom_comu AS Nombre
        FROM Especie
        WHERE nom_comu = %s COLLATE utf8mb4_0900_ai_ci
        LIMIT 1
    """
...
def get_nom_comu_by_id(id_especie):
    cursor.execute("SELECT nom_comu FROM Especie WHERE ID = %s", (id_especie,))
    especie = cursor.fetchone()
    
    if especie:
        return jsonify({"ID": id_especie, "nom_comu": especie["nom_comu"]})
    else:
...
def upload_video():
    ...
    IDVideo = request.form.get("IDVideo", None)

    # decideix carpeta
    if tipus == "directe":
        folder = os.path.join(UPLOAD_FOLDER_DIRECTE, str(request.form.get("IDCamara", 0)))
    else:
        folder = os.path.join(UPLOAD_FOLDER_VIDEO, str(request.form.get("IDUsuario", 0)))
    ...
    if filepath.endswith(".mp4") and tipus == "video":
        try:
            cursor.execute("""
                UPDATE Video
                SET ruta_video = %s
            ...
            conn.rollback()
            return jsonify({"error": f"No s'ha pogut actualitzar la BD: {str(e)}"}), 500
